...
def init_optimizer_state(workload: spec.Workload,
  ...
  def pytorch_cosine_warmup(step_hint: int, step_hint_reduced: int, hyperparameters, optimizer):
    warmup_steps = int(hyperparameters.warmup_factor * step_hint)  # keep same warmup
    warmup = LinearLR(
        optimizer, start_factor=1e-10, end_factor=1., total_iters=warmup_steps)
    cosine_steps = max(step_hint_reduced - warmup_steps, 1)  # reduce cosine steps
  ...
  optimizer_state['scheduler'] = pytorch_cosine_warmup(
      workload.step_hint, 
      workload.step_hint * hyperparameters.pct_budget_reduction, 
      hyperparameters, 
