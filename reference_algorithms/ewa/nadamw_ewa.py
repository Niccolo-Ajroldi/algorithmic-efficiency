...
class EWA():
  def __init__(self, avg_every_steps, ewa_beta):
    self.beta = float(ewa_beta)
    self.ema = None
  
  def update_ewa(self, params):
    """Async update, pin memory?"""
    if self.ema is None:
...
def update_params(
  ...
  # Switch the model and avg after evaluation
  if EWA.just_evaled:
    # switch
  ...
  # Update EWA
  if global_step % avg_interval == 0:
    ewa.ema_update(current_model.parameters())
...
def prepare_for_eval(
  ...
  # Mark the latest step as an eval
  EWA.just_evaled = True
