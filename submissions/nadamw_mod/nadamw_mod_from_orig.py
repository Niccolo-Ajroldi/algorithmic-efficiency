...
Modifications with respect to the original:
- LR scheduler is WarmCosineWithCooldowns (w/ correct inital LR)
- When resuming, input_queue is synced accordingly
...
class WarmCosineWithCooldowns(object):  # WCWC
  def __init__(self, optimizer, lr_min, lr_max, warmup_steps, T, cool_mode, cool_steps=None):
  ...
  def cooldwon_schedule(self, t):
    if t <= self.t_cool_start + self.cool_steps:
      ...
      logging.info("=== REACHED COOLDOWN COMPLETION ===")
  ...
  def step(self):
    self.t += 1
    lr = self.schedule(self.t)  # get LR for this step
    for group in self.optimizer.param_groups:  # set LR in optimizer
      group["lr"] = lr
  ...
  def load_state_dict(self, state_dict):
    state_dict_warmup = state_dict['_schedulers'][0]  # CosineAnnealingLR
    state_dict_cosine = state_dict['_schedulers'][1]  # CosineAnnealingLR
    
    self.warmup_steps = state_dict_warmup["total_iters"]
    
    self.lr_min = state_dict_cosine["eta_min"]
    self.lr_max = state_dict_cosine["base_lrs"][0]
    
    self.t = state_dict["last_epoch"]
    
    # intentionally not loading `cool_mode` and `cool_steps`
    # can also resume without cooling down [NOT HERE!!!]
    if self.cool_mode:
      if self.t <= self.warmup_steps:
          raise NotImplementedError(
              f"Cannot enter cooldown mode during warmup phase. "
              f"Current step: {self.t}, Warmup steps: {self.warmup_steps}."
          )
      self.t_cool_start = self.t
      self.lr_cool_start = self.wc_schedule(self.t_cool_start)
      logging.info(
        f"Resumed, t_cool_start = {self.t_cool_start} "
        f", last_stored_lr = {state_dict_cosine['_last_lr'][0]} "
        f", computed_lr_cool_start = {self.lr_cool_start} "
...
def init_optimizer_state(workload: spec.Workload,
  ...
  optimizer_state['scheduler'] = WarmCosineWithCooldowns(
