"""
Polyak avg on NAdamW optimizer with warmup+cosine LR in PyTorch.
Polyak avg offloaded to cpu.
...
class PolyakAvg():
  ...
  def update_avg(self, params):
    self.count += 1
    if self.avg is None:
      self.avg = [p.detach().cpu() for p in params]
    else:
      k = self.count
      for p_avg, p in zip(self.avg, params):
        p_avg.mul_(k/(k+1)).add_(p.detach().cpu(), alpha=1/(k+1))
...
def init_optimizer_state(workload: spec.Workload,
  ...
  optimizer_state['polyak_avg'] = PolyakAvg(hyperparameters, workload)
...
def update_params(workload: spec.Workload,
  ...
  polyak_avg = optimizer_state['polyak_avg']
  current_model = current_param_container

  # Discard average and load previous params
  if polyak_avg.tmp_params is not None:
    for p, p_old in zip(current_model.parameters(), polyak_avg.tmp_params):
      p.data.copy_(p_old.data)
    polyak_avg.tmp_params = None
  ...
  # Update Polyak Avg
  if global_step >= polyak_avg.start_step and global_step % polyak_avg.every_step == 0:
    polyak_avg.update_avg(current_model.parameters())
...
def prepare_for_eval(workload: spec.Workload,
  ...
  if global_step < polyak_avg.start_step:
    return (optimizer_state, current_model, model_state)

  # Save parameters for next step
  polyak_avg.store_tmp_params(current_model.parameters())

  # Load avg into model
  for p, p_avg in zip(current_model.parameters(), polyak_avg.avg):
