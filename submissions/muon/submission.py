"""Submission file for a Muon optimizer with warmup+cosine LR in PyTorch."""
...
def get_param_groups_muon_dion(model, weight_decay, optim, optim_backup):
  ...
  # bias
  bias_param_names = [n for n in param_names if "bias" in n]
  ...
  # embedding
  embed_param_names = [n for n in param_names if "embed" in n]
  ...
  param_groups = [
    dict(params=matrix_params,  algorithm=optim),
    dict(params=embed_params,   algorithm=optim_backup, weight_decay=weight_decay),
    dict(params=lm_head_params, algorithm=optim_backup, weight_decay=weight_decay),
    dict(params=bias_params,    algorithm=optim_backup, weight_decay=0.0),
    dict(params=norm_params,    algorithm=optim_backup, weight_decay=0.0),
  ]
...
def init_optimizer_state(
  ...
  param_groups = get_param_groups_muon_dion(
      model_params, 
      hyperparameters.weight_decay, 
      hyperparameters.optim, 
      hyperparameters.optim_backup
  )
