...
def update_params(workload: spec.Workload,
  ...
  if RANK == 0:
    timing = {}
    timing["global_step"] = global_step
  ...
  if lawa.tmp_params is not None:
    if RANK == 0:
      ...
      for p, p_old in zip(current_model.parameters(), lawa.tmp_params):
        p.data.copy_(p_old.to(p.device, non_blocking=True))  # Async restore
    
    lawa.tmp_params = None  # Clear after restoring
...
def prepare_for_eval(workload: spec.Workload,
  ...
  # Load avg into model
  stream = torch.cuda.Stream()  # Create a separate CUDA stream for async transfer
  with torch.cuda.stream(stream):  
    for p, p_avg in zip(current_model.parameters(), lawa.ema):
      p.data.copy_(p_avg.to(p.device, non_blocking=True))  # Async transfer to GPU
...
# very cool! what happens if the code reaches a point where it needs the model weights? would it 
